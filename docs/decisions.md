...
## ADR-007: Speech-to-Sign Pipeline Architecture (Streaming VAD/ASR, Topic-Comment Grammar, and SignRepresentation Emitter)
...
Converting continuous spoken English into American Sign Language requires traversing acoustic, phonetic, linguistic, and visual animation layers within a strict conversational latency threshold ($\le 350\text{ms}$).

ASL is fundamentally distinct from signed English:

1. Syntactically, it favors Topic-Comment and Time-Subject-Object-Verb (TSOV) word order over English Subject-Verb-Object (SVO).
2. Grammatical questions and negations require precise non-manual markers (facial blendshapes such as eyebrow furrowing/raising and skeletal head tilting/shaking).
3. Out-of-vocabulary terms (proper nouns, technical jargon) must be decomposed into fingerspelled character animations rather than dropped.
4. Skeletal avatar rendering requires strict timing monotonicity, spatial locus indexing, and co-articulation transitions to prevent unnatural robotic popping.

### Decision

Implement a decoupled, modular Speech-to-Sign model pipeline:

1. **Audio Ingestion & Streaming VAD**: Process 16kHz linear PCM audio chunks using a sliding audio buffer and streaming Voice Activity Detector with 250ms speech lock and 300ms silence hangover flush.
2. **Streaming ASR Engine**: Emit speculative partial transcripts every 200ms for real-time responsiveness, committing authoritative final transcripts on VAD boundary detection.
3. **Linguistic Grammar Compiler**: Transform English syntax into ASL gloss order using deterministic grammatical rule compilation (eliminating copulas, articles, and auxiliary do-support; fronting time adverbials; shifting wh-words to clause-final position; placing negation particles post-clause).
4. **Non-Manual Marker (NMM) Classifier**: Derive synchronized facial blendshape directives (`furrowed` eyebrows for Wh-questions, `raised` eyebrows for Yes/No questions, head shake for negation, and mouth morphemes `cha`, `oo`, `mm`).
5. **Spatial Locus Tracking**: Maintain session-scoped discourse referents across 3D coordinate anchors (`chest`, `neutral_space`, `left`, `right`, `forehead`).
6. **Canonical `SignRepresentation` Emitter**: Output authoritative animation directives conforming to `@converse/contracts`, decomposing OOV words into sub-token fingerspelling clips (`asl_fs_<char>_01`) and computing monotonic lead-in, hold, and lead-out intervals.
7. **Service Integration**: Expose pipeline execution via lightweight internal HTTP microservice (`ml/service.py`) integrated into `services/api` with local rule fallback.

### Consequences

#### Positive

- Total end-to-end compute latency under 2ms for synthetic streaming audio (real-time factor $\ll 0.01$, far beating the 350ms budget).
- Deterministic, auditable grammar reordering free from sequence-to-sequence hallucinations.
- Strict contract conformance with `@converse/contracts` guaranteeing out-of-the-box compatibility with downstream WebGL avatar engines.
- Additive schema evolution without breaking existing endpoints or types.
